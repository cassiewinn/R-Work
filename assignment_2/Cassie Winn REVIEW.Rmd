...
#I would suggest also looking at dimensions of the files, number of columns and number of lines
...
```
5) Data sets need to sorted before they can be joined.
```{r}
...
# Didn't know this command, that's cool
I) Maize
```{r}
library(gtools)
...
# Good. I used a for loop instead but this way works great too!
```{r}
library(dplyr)
